...
class OreChunkProvider:
    ...
    def generate(self):
        ...
        for x, z in self.chunkprovider.generationcache["biomemap"].keys():
            biome = G.biomehandler.biomes[self.chunkprovider.generationcache["biomemap"][(x, z)]]
        ...
        for biome in biomes:
            ...
            for vein in oreveins:
                if not vein in self.generationtable:
                    ...
                    veintable[vein] = biomeamount[biome] * vein.getChunkAmount(vein) / 256
                else:
                    veintable[vein] += biomeamount[biome] * vein.getChunkAmount(vein) / 256
        for orevein in self.generationtable.keys():
            while self.generationtable[orevein] < veintable[orevein]:
                x = self.random.generateValueForPosition((self.chunk[0] * 16, -12, self.chunk[1] * 16), 0, 15)
                x += self.chunkprovider.chunk[0] * 16
                z = self.random.generateValueForPosition((self.chunk[0] * 16, -14, self.chunk[1] * 16), 0, 15)
                z += self.chunkprovider.chunk[1] * 16
                cx, _, cz = mathhelper.sectorize((x, 0, z))
                chunkprovider = self.chunkprovider.worldprovider.getChunkProviderFor((cx, cz))
                if chunkprovider.generated:
                    y = self.random.generateValueForPosition((x, -13, z), 0,
...
class OreVein:
    ...
    def getChunkAmount(self): return 0
...
class BaseOreVein(OreVein):
    def past(self, chunkprovider, r, x, y, z):
        ...
        xlenght = r.generateValueForPosition((x, y - 1003, z), self.getMinSize(self) / 2, self.getMaxSize(self) / 2)
        ylenght = r.generateValueForPosition((x, y - 1004, z), self.getMinSize(self) / 2, self.getMaxSize(self) / 2)
        zlenght = r.generateValueForPosition((x, y - 1005, z), self.getMinSize(self) / 2, self.getMaxSize(self) / 2)
        if xlenght == 0 or ylenght == 0 or zlenght == 0: return #this orevein is empty
        for dx in range(-xlenght, xlenght+1):
            for dy in range(-ylenght, ylenght+1):
                for dz in range(-zlenght, zlenght+1):
                    ...
                    if (mx * mx) / (xlenght * xlenght) + \
                            (my * my) / (ylenght * ylenght) + \
                            (mz * mz) / (zlenght * zlenght) <= 1: #valid position
                        ...
                        if chunkprovider.generated:
                            chunkprovider.generationcache["blocks"][(x+dx, y+dy, z+dz)] = self.getOreName(self)

    def getOreName(self): return ""

    def getMinSize(self): return 1

    def getMaxSize(self): return 5

    def getChunkAmount(self): return 0

    def getReplaceAbleBlocks(self): return ["minecraft:stone"]


class CoalOre(BaseOreVein):
    def getOreName(self): return "minecraft:coal_ore"
    def getMinSize(self): return 0
    def getMaxSize(self): return 64
    def getChunkAmount(self): return 10
