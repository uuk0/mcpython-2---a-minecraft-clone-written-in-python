...
class Saver(storage.saver.Saver):
    ...
    def saveWorld(self, file):
        ...
        worldinfo = {"version_name": G.VERSION_NAME, "version_id": G.VERSION_ID,
                     "main_player_name": G.player.name, "seed": G.seed, "storage_version": self.getStorageVersion(),
                     "player_table": {G.player.name: {"position": G.window.position,
        ...
        for id in G.inventoryhandler.inventorys.keys():
            ...
            if issubclass(type(inventory), Inventory.inventory.Inventory):
                ...
                for slot in inventory.slots:
                    if slot.stack.item:
                        inventorydata["slots"].append({"item": slot.stack.item.getName(),
                ...
                inventorydata = {"type": "INVENTORYCOLLECTION",
        ...
        for entity in G.entityhandler.entitys:
            entitydata = {"position": entity.position, "rotation": None,
                          "name": entity.getName(), "nbt": entity.getNBT()}
    ...
    def saveDim(self, dim, file):
        ...
        for chunk in worldprovider.chunkproviders.keys():
            self.saveChunk(chunk[0], chunk[1], file+"/c."+str(round(chunk[0]))+"."+str(round(chunk[1]))+".chunk")

    def saveChunk(self, cx, cz, file):
        chunkprovider = G.player.dimension.worldprovider.getChunkProviderFor((cx, cz))
        if len(chunkprovider.world) == 0: return
        log.printMSG("[SAVER][0.0.1][INFO] collecting chunk "+str((cx, cz)))
        chunkdata = {"saved_version": G.VERSION_ID, "position": (cx, cz),
                     "blocks": {}, "inventorybindings": {}}
        for position in chunkprovider.world.keys():
            chunkdata["blocks"][position] = chunkprovider.world[position].getStorageData()
            inventorys = chunkprovider.world[position].getInventorys()
            if len(inventorys) > 0:
                chunkdata["inventorybindings"][position] = [inventory.id for inventory in inventorys]
...
class Loader(storage.loader.Loader):
    ...
    def loadWorld(self, file):
        ...
        if G.VERSION_ID < worlddata["version_id"]:
            log.printMSG("can't load world. world is saved in an NEWER version of mcpython")
            raise RuntimeError()
        ...
        G.player.selectedinventoryslot = worlddata["player_table"][G.player.name]["selectedinventoryslot"]
        ...
        self.loadDim(worlddata["player_table"][G.player.name]["dimension"], file + "/DIM" +
                     str(worlddata["player_table"][G.player.name]["dimension"]))
    ...
    def loadDim(self, dim, file):
        ...
        for f in os.listdir(file):
            if f.endswith(".chunk"):
                ...
                cx, cz = int(cx), int(cz)
                self._loadChunk(worldprovider.getChunkProviderFor((cx, cz)), file+"/"+f)

    def loadChunk(self, cx, cz, file):
        chunkprovider = G.player.dimension.worldprovider.getChunkProviderFor((cx, cz))
        self._loadChunk(chunkprovider, file+"/c."+str(cx)+"."+str(cz)+".chunk")
    ...
    def _loadChunk(self, chunkprovider, file):
        ...
        for position in chunkdata["blocks"].keys():
            if chunkdata["blocks"][position]["shown"]:
                G.model.add_block(position, chunkdata["blocks"][position]["name"],
                                  immediate=True)
                chunkprovider.world[position].setStorageData(chunkdata["blocks"][position])
            else:
                G.BlockGenerateTasks[position] = [chunkdata["blocks"][position]["name"], "sdata",
                                                  chunkdata["blocks"][position]]

    def isFile(self, file):
        with open(file + "/world.info", mode="r") as f:
            worlddata = json.load(f)
        return "storage_version" in worlddata and worlddata["storage_version"] == list(self.getStorageVersion())
