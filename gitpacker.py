...
if not "C:/Python/mcpython/mcpython_5" in sys.path:
    sys.path.append("C:/Python/mcpython/mcpython_5")
...
def _getFullFile(file): #input splitted by "."
    ...
    fd = None
    for d in [os.path.dirname(sys.argv[0])]:
...
def pack(file, out, res=[], not_res=[], not_res_end=[], version=0, name="", local=None, syspath=None):
    with open("./execption.txt", mode="w") as f: f.write("")
...
def pack_mcpython(f=".", to=".", version=None):
    ...
    sysdata = {"version_id":version, "gamename":"mcpython",
                     "modable":True, "version_name":input("select an version-name: ")}
    with open("./gameinfo.dat", mode="wb") as file:
        pickle.dump(sysdata, file)
    for e in os.listdir(to):
        if os.path.isfile(to+"/"+e) and e.endswith(".spart"):
            os.remove(to+"/"+e)
    pack([f+"/main.py"],
                to, res=[f+"/assets", f+"/Block", f+"/Item", f+"/Inventory",
               f+"/version.info", f+"/gameinfo.dat", f+"/mods/file.info",
                         f+"/tmp/file.info", "./entity/entity.py",
                         "./entity/boxmodel.py", "./notations.py",
                         "./biomes", "./command", "./lang"],
         not_res=[],
         not_res_end=[".pyc", ".xml"],
         version=version,
         name="mcpython",
         local=[f], syspath=sys.path[:])
    with zipfile.ZipFile("C:/Python/mcpython/mcpython_5_backups/"+sysdata["version_name"]+".zip", mode="w") as f:
        for e in os.listdir(to):
            if os.path.isfile(to + "/" + e) and e.endswith(".spart"):
                f.write(e, to+"/"+e)
