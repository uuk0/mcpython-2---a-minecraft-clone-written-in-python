...
class TextureAtlasHandler:
    ...
    def _add_file_collection(self, files, raw_indexes):
        indexes = []
        files = [textures.util.join_file(file) if type(file) == str else file for file in files]
        if len(files) > 256: raise ValueError("can't add more than 256 images to an image atlas")
                ...
                for file in files:
                    if type(file) == str:
                        ...
                        if file not in textureatlas.files:
                            indexes.append(textureatlas.add_image(textures.util.load_image(file)))
        ...
        for file in files:
            textures.util.resize_file(file, (64, 64))
            indexes.append([textureatlas.textureatlasid, textureatlas.add_image(textures.util.load_image(file) if
            type(file) == str else file)])
            textureatlas.files[file] = indexes[-1]
        for i, index in enumerate(indexes):
            self.indexarray[raw_indexes[i]] = index
        return indexes
    ...
    def _add_file(self, file):
        for textureatlas in self.atlases:
            if textureatlas.free_places > 0:
                ...
                return textureatlas.add_image(textures.util.load_image(file) if type(file) == str else file)
        textureatlas = TextureAtlas(len(self.atlases))
        self.atlases.append(textureatlas)
        return textureatlas.textureatlasid, textureatlas.add_image(textures.util.load_image(file) if type(file) == str
                                                                   else file)
    ...
    def generate(self):
        ...
        for i, textureatlas in enumerate(self.atlases):
            textureatlas.baseimage.save(G.local+"/tmp/texture_atlas_"+str(i)+".png")
            textureatlas.pyglet_atlas = pyglet.graphics.TextureGroup(pyglet.image.load(
                G.local+"/tmp/texture_atlas_"+str(i)+".png").get_texture())
...
class TextureAtlas:
    def __init__(self, textureatlasid: int):
        ...
        self.baseimage = PIL.Image.new("RGBA", (16*TEXTURE_ENTRY_SIZE, 16*TEXTURE_ENTRY_SIZE), (255, 255, 255, 255))
        nondefimage = textures.util.load_image(G.local + "/assets/minecraft/textures/missingtexture.png")
        nondefimage = textures.util.resize(nondefimage, (TEXTURE_IMAGE_SIZE, TEXTURE_IMAGE_SIZE))
        for x in range(16):
            for y in range(16):
                image = nondefimage.crop((0, 0, TEXTURE_IMAGE_SIZE, TEXTURE_IMAGE_SIZE))
                d = PIL.ImageDraw.Draw(image)
                d.multiline_text((10, 10), str((x+(15-y)*16)) + "\n" + str(self.textureatlasid), fill=255)
                self.baseimage.paste(image, (x*TEXTURE_ENTRY_SIZE, y*TEXTURE_ENTRY_SIZE))
        self.nextindex = [0, 0]
    ...
    def add_image(self, image: PIL.Image.Image) -> tuple:
        ...
        if self.nextindex[0] > 15:
            self.nextindex[0] = 0
            self.nextindex[1] += 1
        self.baseimage.paste(image, (index[0]*TEXTURE_ENTRY_SIZE, (15-index[1])*TEXTURE_ENTRY_SIZE))
